...
python .claude/skills/op-crawler/query.py jobs                 # all crawler jobs (id, state, progress)
python .claude/skills/op-crawler/query.py job <job_id>         # one job's detail
python .claude/skills/op-crawler/query.py run <job_id>         # the pipeline run that owns this job (+ log tail with --tail)
python .claude/skills/op-crawler/query.py frontier-preview --sample 25   # what the next round would fetch

# ── Control (admin password) ──
python .claude/skills/op-crawler/query.py pause <job_id>
python .claude/skills/op-crawler/query.py resume <job_id>
python .claude/skills/op-crawler/query.py cancel <job_id>
python .claude/skills/op-crawler/query.py delete <job_id>

# Node equivalent (same subcommands + flags)
node .claude/skills/op-crawler/query.mjs jobs
```
