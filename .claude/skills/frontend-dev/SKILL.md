...
2. Cross-reference `CLAUDE.md` for general repo conventions and the **UI verification rule** (every visible change must be exercised through the Playwright MCP server against the running dev server).
...
When you scaffold `src/your-web/` (or restyle an existing app), apply the system in this order. Each step is a verifiable change you can confirm with a Playwright MCP screenshot (per the UI-verification rule in `CLAUDE.md`).
