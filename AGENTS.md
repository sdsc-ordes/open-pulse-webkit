...
This repo ships an MCP-server config for **Playwright MCP** (`@playwright/mcp`) in `.mcp.json` at the repo root, enabled via `.agents/settings.json` (`enabledMcpjsonServers: ["playwright"]`). It is port-locked to the local dev/preview servers.
