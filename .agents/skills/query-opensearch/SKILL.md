...
```
.agents/skills/query-opensearch/
...
Cross-store: SPARQL and Cypher use `https://github.com/{owner}/{repo}` with no suffix; CHAOSS and collections take `owner` + `repo` separately. See `.agents/SKILLS.md` §12.
...
python .agents/skills/query-opensearch/query.py 'SELECT hash, author_date FROM git ORDER BY author_date DESC LIMIT 5'

# Count docs in an index
python .agents/skills/query-opensearch/query.py 'SELECT count(*) FROM git_demo_enriched'

# Discover an index's fields (DESCRIBE is empty for readers)
python .agents/skills/query-opensearch/query.py 'SELECT * FROM git_demo_enriched LIMIT 1'

# From stdin or a file
echo 'SELECT count(*) FROM git' | python .agents/skills/query-opensearch/query.py -
python .agents/skills/query-opensearch/query.py -f path/to/query.sql
...
python .agents/skills/query-opensearch/query.py --raw --dsl git_demo_enriched -f body.json

# Node equivalent (same flags)
node .agents/skills/query-opensearch/query.mjs 'SELECT count(*) FROM git'
```
