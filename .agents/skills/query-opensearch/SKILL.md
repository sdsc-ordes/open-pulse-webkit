...
```bash
# Cluster sanity
python .agents/skills/query-opensearch/query.py health

# List indices (pattern optional, defaults to *)
python .agents/skills/query-opensearch/query.py indices
python .agents/skills/query-opensearch/query.py indices 'github*'

# Count docs in an index
python .agents/skills/query-opensearch/query.py count git

# DSL search — body inline, from stdin, or from a file
python .agents/skills/query-opensearch/query.py search git '{"query":{"match_all":{}}}'
echo '{"query":{"term":{"author_name":"alice"}}}' | python .agents/skills/query-opensearch/query.py search git -
python .agents/skills/query-opensearch/query.py search git -f path/body.json --size 50

# Arbitrary GET — fall-through for endpoints the wrapper doesn't model
python .agents/skills/query-opensearch/query.py get /_cluster/state?filter_path=metadata.indices.*.settings

# Node equivalent (same flags)
node .agents/skills/query-opensearch/query.mjs indices
```
