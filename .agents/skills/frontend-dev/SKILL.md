...
3. Cross-reference `AGENTS.md` for the **UI verification rule** (every visible change must be exercised through the Playwright MCP browser against the running dev server).
...
When you scaffold `src/your-web/` (or restyle an existing app), apply in this order — each step verifiable with a Playwright MCP screenshot (per `AGENTS.md`):
